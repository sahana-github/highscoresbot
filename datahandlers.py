...
class Worldbossdatahandler:
    ...
    def addworldboss(self, worldboss):
        ...
        datacur.execute("INSERT INTO worldboss(worldbossname, date) VALUES(?,?)",
                        (worldboss, str(datetime.datetime.now(tz=datetime.timezone.utc)).split(" ")[0]))
        self.datadb.commit()

    def addplayers(self):
        datacur = self.datadb.cursor()
        highscorescur = self.highscoresdb.cursor()
        id = datacur.execute("SELECT id FROM worldboss ORDER BY id DESC LIMIT 1").fetchall()[0][
            0]  # get the id from the last worldboss
        # return 0
        ...
        if len(datacur.execute("SELECT * FROM worldbossdmg WHERE worldbossint = ?", (id,)).fetchall()) < 1000:
            print("insert called")
            for i in result:
                datacur.execute("INSERT INTO worldbossdmg(playername, totaldamage, worldbossint) VALUES(?,?,?)",
                                (i[0], int(i[1].replace(",", "")), id))
            self.datadb.commit()
        else:
            print("update called")
            for i in result:
                datacur.execute("UPDATE worldbossdmg SET totaldamage = ? WHERE playername = ? and worldbossint = ?",
                                (int(i[1].replace(",", "")), i[0], id))
            self.datadb.commit()

    def calcdifference(self, worldbossnumber):
        datacur = self.datadb.cursor()
        before = datacur.execute("SELECT playername, totaldamage FROM worldbossdmg WHERE worldbossint = ?",
                                 (worldbossnumber - 1,)).fetchall()
        after = datacur.execute("SELECT playername, totaldamage FROM worldbossdmg WHERE worldbossint = ?",
                                (worldbossnumber,)).fetchall()
        beforedict = {}
        for i in before:
    ...
    def calcallworldbosses(self):
        datacur = self.datadb.cursor()
        id = datacur.execute("SELECT id FROM worldboss ORDER BY id DESC LIMIT 1").fetchall()[0][
            0]  # get the id of the last world boss
        alldicts = []
        ...
        for i in alldicts:
            try:
                if i[playername] != 0:
                    ...
                    datalist.append([temp[0], i[playername], temp[1], list(i).index(playername) + 1,
                                     len({j for j in i.items() if j[1] != 0})])
            except KeyError:
                pass
            count += 1
        return datalist
